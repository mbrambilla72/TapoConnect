...
namespace TapoConnect.Protocol
{
    public class SecurePassthroughDeviceClient : ITapoDeviceClient
    {
        ...
        public virtual async Task<PlugResult> GetEnergyUsageAsync(TapoDeviceKey deviceKey)
        {
            ...
            var response = await SecurePassthroughAsync<ResultPlugResponse>(jsonRequest, deviceKey.DeviceIp, protocol.Token, deviceKey.SessionCookie, protocol.Key, protocol.Iv);
